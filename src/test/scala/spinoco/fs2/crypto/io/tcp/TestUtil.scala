...
object TestUtil {
  ...
  /**
    ...
    *
    */
  def localTcpSocket[F[_]](received: Stream[F, Byte])(implicit F: Async[F]): Stream[F, (Socket[F], Stream[F, Byte])] = {
    ...
    Stream.eval(async.signalOf[F, Received](Received(Chunk.empty, false, Vector.empty))) flatMap { receivedSignal =>
      ...
      val enqueue: Stream[F, Unit] = (received.chunks.evalMap { offer } onFinalize  closedInput) interruptWhen receivedSignal.discrete.map(_.closed)
